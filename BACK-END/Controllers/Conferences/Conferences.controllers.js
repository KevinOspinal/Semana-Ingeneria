...
const getConferences = (req, res) => {
  conexion.query('SELECT c.id_conferencia ,c.descipcion, s.nombre_sede, c.cupo, c.fecha , c.hora, c.estado_conferencia FROM  tb_sedes s, tb_conferencias c WHERE  s.id_sede = c.id_sede',
    (err, result) => {
      if (err) {
        console.log(err)
